...
WITH DuplicateRecords AS (
    SELECT
        JourneyID,  -- Select the unique identifier for each journey (and any other columns you want to include in the final res
...
(
        ...
        SELECT 
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY JourneyID
            ) AS row_num -- Assigns a row number to identify duplicates
        FROM 
            dbo.customer_journey
    ) AS subquery -- Names the subquery for reference in the outer query
where 
    row_num = 1;
    --Keeps only the first  occurences of each duplicates group identified in the subquery